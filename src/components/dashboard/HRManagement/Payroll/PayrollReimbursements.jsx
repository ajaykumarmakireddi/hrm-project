          ...
          <div className={styles.attachList}>
            {form.attachments.map((a, i) => (
              <div key={i} className={styles.attachItem}>
                <div>
                  {a.name} <small>({Math.round(a.size / 1024)} KB)</small>
                </div>
                <button
                  type="button"
    ...
    reader.onload = () => {
      try {
        ...
        const pv = parsed.map((r, idx) => {
          const item = {
            employeeName:
              r["employee name"] || r["employee_name"] || r["employee"],
            category: r["category"],
          ...
          <div className={styles.previewList}>
            {preview.slice(0, 50).map((p) => (
              <div key={p.row} className={styles.previewRow}>
                <div>
                  <strong>Row {p.row}</strong> — {p.item.employeeName} —{" "}
                  {p.item.category} — {p.item.amount}
                </div>
                {p.errors.length > 0 ? (
                  <div className={styles.previewErr}>{p.errors.join(", ")}</div>
                ) : (
                  <div className={styles.previewOk}>OK</div>
                )}
              </div>
            ))}
          </div>
      ...
      {errors.length > 0 && (
        <div className={styles.bulkErrors}>
          ...
          {errors.slice(0, 10).map((e, i) => (
            <div key={i}>
              Row {e.row}: {e.errors ? e.errors.join(", ") : e.message}
            </div>
          ))}
        </div>
      )}
    ...
    <div className={styles.drawerBackdrop} onClick={onClose}>
      <div className={styles.drawer} onClick={(e) => e.stopPropagation()}>
        <div className={styles.drawerHeader}>
          ...
          <div>
            <strong>{formatCurrency(claim.amount, claim.currency)}</strong>
          </div>
        </div>
        <div className={styles.drawerBody}>
          <div>
            <strong>Employee</strong>: {claim.employeeName} ({claim.employeeId})
          </div>
          <div>
          ...
          <div>
            <strong>Approver</strong>: {claim.approver || "—"}
          </div>
          <div>
            <strong>Notes</strong>:{" "}
            <div className={styles.notes}>{claim.notes}</div>
          </div>
          <div>
        ...
        <div className={styles.drawerActions}>
          {claim.status === "Pending" && (
            <>
              <button className={styles.btn} onClick={() => onReject(claim.id)}>
                Reject
              </button>
              <button
                className={styles.btnPrimary}
...
export default function PayrollReimbursements({ navigate }) {
  ...
  async function handleUploadBulk(list) {
    ...
    if (resp.ok) {
      setToast({
        type: "success",
        msg: `Uploaded ${resp.created.length} claims`,
      });
      await load();
    } else {
  ...
  async function handleExportCSV() {
    const resp = await api.exportClaimsCSV({
      filter: {
        status: filterStatus || undefined,
        category: filterCategory || undefined,
      },
    ...
    if (resp.ok) {
      ...
      a.href = url;
      a.download = `reimbursements_${Date.now()}.csv`;
      a.click();
      ...
      <div className={styles.page}>
        <div className={styles.headerRow}>
          <div className={styles.headerActions}>
            <button className={"homebtn"} onClick={() => setShowNewModal(true)}>
              New Claim
            </button>
            <button
              className={"homebtn"}
            ...
            <input
              ...
              onChange={(e) => {
                ...
                reader.onload = async () => {
                  ...
                  if (lines.length < 2) {
                    setToast({
                      type: "error",
                      msg: "CSV must have header + at least 1 row",
                    });
                    setTimeout(() => setToast(null), 1500);
                    return;
                  }
                  ...
                  const valid = parsed.filter(
                    (p) => p.employeeName && p.category && p.amount > 0
                  );
                  if (valid.length === 0) {
                    setToast({ type: "error", msg: "No valid rows in CSV" });
                    setTimeout(() => setToast(null), 1500);
                    return;
                  }
                  ...
                  if (res.ok) {
                    setToast({
                      type: "success",
                      msg: `Uploaded ${res.created.length} claims`,
                    });
                    await load();
                    setTimeout(() => setToast(null), 1500);
                  }
              ...
              style={{ display: "none" }}
            />
            <button className={"homebtn"} onClick={handleExportCSV}>
              Export CSV
            </button>
          </div>
        </div>
        ...
        <div className={"d-flex justify-content-center"}>
          <div className={`${styles.summaryStrip} w-75`}>
            <div>
              Total claims (page): <strong>{totals.totalCount}</strong>
            </div>
            <div>
              Total amount (page):{" "}
            ...
            <div>
              Pending: <strong>{totals.byStatus["Pending"] || 0}</strong>
            </div>
            <div>
              Approved: <strong>{totals.byStatus["Approved"] || 0}</strong>
            </div>
            <div>
              Rejected: <strong>{totals.byStatus["Rejected"] || 0}</strong>
            </div>
          </div>
        </div>
        ...
        <div className={styles.tableWrap}>
          {loading ? (
            ...
            <>
              ...
              <div className={"d-flex justify-content-center"}>
                <div className={`${styles.pager} w-75`}>
                  <button
                    className={styles.btn}
