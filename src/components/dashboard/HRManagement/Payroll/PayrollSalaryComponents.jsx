...
const api = {
  async listCountries() {
    await mockDelay();
    return mockDB.countries.slice();
  },
  async listGroups() {
    await mockDelay();
    return mockDB.groups.slice();
  },
  async listComponents(filter = {}) {
    await mockDelay();
    let list = mockDB.components.slice();
    if (filter.country)
      list = list.filter((c) => c.countries.includes(filter.country));
    if (filter.category)
  ...
  async updateComponent(id, payload) {
    ...
    const updated = {
      ...mockDB.components[idx],
      ...payload,
      history: (mockDB.components[idx].history || []).concat({
...
// -------------------- Helpers --------------------
const uniqueCode = (name) =>
  name.trim().toUpperCase().replace(/\s+/g, "_").slice(0, 20);
  ...
  return (
    <div className={styles.header}>
      ...
      <div className={styles.headerRight}>
        ...
        <button className={styles.btn} onClick={onExportAll}>
          Export CSV
        </button>
      </div>
    </div>
...
function CategoryTabs({ active, setActive }) {
  ...
  return (
    <>
      {tabs.map((t) => (
        <button key={t} className={`homebtn`} onClick={() => setActive(t)}>
          {t}
        </button>
      ))}
    </>
          ...
          {data.map((c) => (
            <tr key={c.id}>
              ...
              <td>
                {c.calcMode === "Fixed"
                  ? c.calcValue
                  : c.calcMode === "%base" || c.calcMode === "%gross"
                  ? `${c.calcValue}%`
                  : c.calcMode === "Formula"
                  ? "Formula"
                  : String(c.calcValue || "")}
              ...
              <td>
                <span
                  className={`${styles.statusTag} ${
                    styles["status_" + c.status.toLowerCase()]
                  }`}
                >
                  {c.status}
                </span>
              ...
              <td className={styles.actionsCell}>
                <button className={"table-view-btn"} onClick={() => onView(c)}>
                  View
                </button>
                <button
...
function ComponentModal({ open, initial, onClose, onSave, countries }) {
  ...
  useEffect(() => {
    if (initial) setForm({ ...initial });
      ...
      setForm((f) => ({ ...f, countries: [countries?.[0]].filter(Boolean) }));
    }
  }, [initial, countries]);
  ...
  function validate() {
    ...
    if (
      (form.calcMode === "Fixed" || form.calcMode.indexOf("%") >= 0) &&
      (form.calcValue === "" || isNaN(Number(form.calcValue)))
    )
      e.calcValue = "Numeric value required";
    ...
    <div className={styles.modalBackdrop} onClick={onClose}>
      ...
      >
        <div className={styles.modalHeader}>
          <h3>
            {form.id ? `Edit Component — ${form.name}` : "Create Component"}
          </h3>
          <div>
        ...
        <div className={styles.modalBody}>
          <div className={styles.leftCol}>
            ...
            <input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  name: e.target.value,
                  code: uniqueCode(e.target.value),
                })
              }
            />
            {errors.name && (
            ...
            <input
              className={styles.input}
              value={form.code || uniqueCode(form.name)}
            ...
            <select
              className={styles.input}
              value={form.calcMode}
              onChange={(e) => setForm({ ...form, calcMode: e.target.value })}
            >
              {calcModes.map((m) => (
            ...
            {errors.calcMode && (
              <div className={styles.fieldError}>{errors.calcMode}</div>
            )}
            ...
            <input
              ...
              onChange={(e) => setForm({ ...form, calcValue: e.target.value })}
              placeholder="Enter number or formula (if Formula mode)"
            />
            {errors.calcValue && (
              <div className={styles.fieldError}>{errors.calcValue}</div>
            )}
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, taxable: e.target.value === "Yes" })
              }
            >
            ...
            <input
              className={styles.input}
              value={form.statutoryLink || ""}
              onChange={(e) =>
                setForm({ ...form, statutoryLink: e.target.value })
              }
              placeholder="e.g., PF, ESI, PT"
            />
          ...
          <div className={styles.rightCol}>
            ...
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.behavior.proration}
                onChange={(e) =>
                  setForm({
                    ...form,
                    behavior: { ...form.behavior, proration: e.target.checked },
                  })
                }
              />{" "}
              Proration (prorate on join/exit)
            </label>
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.behavior.dependsOnAttendance}
                onChange={(e) =>
                  setForm({
                    ...form,
                    behavior: {
                      ...form.behavior,
                      dependsOnAttendance: e.target.checked,
                    },
                  })
                }
              />{" "}
            ...
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.behavior.includedInGross}
                onChange={(e) =>
                  setForm({
                    ...form,
                    behavior: {
                      ...form.behavior,
                      includedInGross: e.target.checked,
                    },
                  })
                }
              />{" "}
            ...
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.behavior.includedInCTC}
                onChange={(e) =>
                  setForm({
                    ...form,
                    behavior: {
                      ...form.behavior,
                      includedInCTC: e.target.checked,
                    },
                  })
                }
              />{" "}
            ...
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.behavior.showInPayslip}
                onChange={(e) =>
                  setForm({
                    ...form,
                    behavior: {
                      ...form.behavior,
                      showInPayslip: e.target.checked,
                    },
                  })
                }
              />{" "}
            ...
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.behavior.lockAfterCycle}
                onChange={(e) =>
                  setForm({
                    ...form,
                    behavior: {
                      ...form.behavior,
                      lockAfterCycle: e.target.checked,
                    },
                  })
                }
              />{" "}
              Lock editing after cycle
            </label>
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.behavior.carryForward}
                onChange={(e) =>
                  setForm({
                    ...form,
                    behavior: {
            ...
            <p className={styles.muted}>
              Configure region-specific rules in production; here we store a
              label for mapping.
            </p>
            ...
            <input
              ...
              onChange={(e) => setForm({ ...form, regionKey: e.target.value })}
              placeholder="e.g., PF_RULE_IND"
            />
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          <div className={styles.detailCol}>
            <h4>Summary</h4>
            <div>
              <strong>Code</strong>: {component.code}
            </div>
            <div>
            ...
            <div>
              ...
              {(component.countries || []).join(", ")}
            </div>
            <div>
              <strong>Calc Mode</strong>: {component.calcMode} —{" "}
              {component.calcMode === "Formula"
                ? component.calcValue
                : component.calcMode === "%base" ||
                  component.calcMode === "%gross"
                ? `${component.calcValue}%`
                : component.calcValue}
            </div>
            <div>
            ...
            <div>
              <strong>Statutory Link</strong>: {component.statutoryLink || "—"}
            </div>
          </div>
          ...
          <div className={styles.detailCol}>
            ...
            <pre className={styles.pre}>
              {JSON.stringify(component.behavior || {}, null, 2)}
            </pre>
            <h4>Usage in Structures</h4>
            <p className={styles.muted}>
              This would show where the component is used. (Mocked)
            </p>
            <h4>History</h4>
            <div className={styles.historyList}>
              {(component.history || []).length === 0 && (
                <div className={styles.muted}>No history entries</div>
              )}
              {(component.history || []).map((h, i) => (
...
export default function PayrollSalaryComponents({ navigate }) {
  ...
  function onImportClick(e) {
    ...
    reader.onload = async () => {
      ...
      if (lines.length < 2) {
        setToast("CSV must include header + rows");
      ...
      const errs = [];
      rows.forEach((r, idx) => {
        if (!r.name || !r.code || !r.calcmode)
          errs.push({ row: idx + 2, err: "name/code/calcMode required" });
        else
          valid.push({
            name: r.name,
            code: r.code,
            ...
            countries: (r.countries || "")
              ...
              .filter(Boolean),
            calcMode: r.calcmode,
            calcValue: r.calcvalue || 0,
            taxable: (r.taxable || "no").toLowerCase().startsWith("y"),
            statutoryLink: r.statutorylink || "",
            visibility: r.visibility || "Payslip",
            status: "Active",
          });
      });
      if (errs.length) {
        setToast(`CSV had ${errs.length} errors (first: row ${errs[0].row})`);
        setTimeout(() => setToast(null), 1800);
      }
      // create valid
      for (const v of valid) await api.createComponent(v);
      setToast(`Imported ${valid.length} components`);
  ...
  async function onExportAll() {
    ...
    a.href = url;
    a.download = `salary-components-${Date.now()}.csv`;
    a.click();
  ...
  async function onArchive(c) {
    const res = await api.updateComponent(c.id, {
      status: "Archived",
    ...
    <>
      ...
      <div className={styles.page}>
        <HeaderToolbar
          ...
          onCreate={onCreate}
          onImportClick={(e) => onImportClick(e)}
